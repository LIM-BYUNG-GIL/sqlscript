...
SELECT pname FROM TBL_PRODUCT tp		-- 외부쿼리
	...
	(SELECT PCODE FROM TBL_BUY tb WHERE custom_id = 'mina012' AND buy_date = '2022-2-6');		-- 내부 쿼리
...
-- 서브쿼리 문제점 : 외부쿼리가 조건식을 모든행에 대해 검사할때마다 내부쿼리가 실행되므로 처리 속도에 문제가 생깁니다.
-- --> 테이블의 조인 연산 사용으로 개선합니다.

-- 2. SELECT 의 테이블 JOIN :둘 이상의 테이블(주로 참조관계의 테이블)을 연결하여 데이터를 조회하는 명령
-- 동등 조인 : 둘 이상의 테이블은 공통된 컬럼을 갖고 이 컬럼값이 '같다(=)'를 이용하여 join 합니다.
-- 형식 1 : select ~~~ from 테이블1 a, 테이블2 b WHERE a.공통컬럼1=b.공통컬럼1;
...
SELECT * FROM TBL_PRODUCT tp
		JOIN TBL_BUY tb				-- 동등 조인. 조인 컬럼으로 = 연산식.
...
-- 조인할때, 이름이 같은 칼럼은 테이블명을 꼭 지정해야 합니다.
...
-- 3. 외부 조인 (outer join) : = 연산을 사용하는 조인이나 한쪽에 없는 값(null)도 조인결과로 포함.
-- JOIN 키워드 없는 형식 1
SELECT * FROM TBL_PRODUCT tp , TBL_BUY tb 
	WHERE tp.PCODE = tb.PCODE(+) ;
	-- 외부조인 tbl_buy 테이블에 일치하는 pcode 값이 없어도 조인. 조인했을때 null 이 되는 테이블의 컬럼에 (+) 기호
